...
from jakarta_analyze.modules.utils.os import syscall_decode as sp
from jakarta_analyze.modules.data.database_io import DatabaseIO as DBIO
# ============== Logging  ========================
import logging
...
class VideoFile:
    ...
    def set_paths(self, path):
        ...
        logger.info("current config: %s", conf)
        self.frame_stats_path = os.path.join(conf.get('dirs', {}).get('frame_stats', 'outputs/frame_stats/'), self.name + ".csv")
        self.packet_stats_path = os.path.join(conf.get('dirs', {}).get('packet_stats', 'outputs/packet_stats/'), self.name + ".csv")
        self.raw_subtitles_path = os.path.join(conf.get('dirs', {}).get('subtitles', 'outputs/subtitles/'), self.name + ".srt")
        self.subtitles_path = os.path.join(conf.get('dirs', {}).get('subtitles', 'outputs/subtitles/'), self.name + ".csv")
        self.errors_path = os.path.join(conf.get('dirs', {}).get('errors', 'outputs/errors/'), self.name + ".csv")
    ...
    def extract_frame_stats(self,
        ...
        if self._frames_extracted and (not forcenew):
            ...
            return 0
        if not os.path.exists(conf.get('dirs', {}).get('frame_stats', 'outputs/frame_stats/')):
            os.makedirs(conf.get('dirs', {}).get('frame_stats', 'outputs/frame_stats/'))
    ...
    def extract_packet_stats(self, entries="pts_time,dts_time,size,pos,flags", forcenew=False):
        if self._packets_extracted and (not forcenew):
            return 0
        if not os.path.exists(conf.get('dirs', {}).get('packet_stats', 'outputs/packet_stats/')):
            os.makedirs(conf.get('dirs', {}).get('packet_stats', 'outputs/packet_stats/'))
        stdout, stderr, returnstatus = sp(['ffprobe', "-loglevel", "panic", "-of", "csv=p=0", "-select_streams", "v",
    ...
    def extract_subtitles(self, forcenew=False):
        if self._subtitles_extracted and (not forcenew):
            return 0
        if not os.path.exists(conf.get('dirs', {}).get('subtitles', 'outputs/subtitles/')):
            os.makedirs(conf.get('dirs', {}).get('subtitles', 'outputs/subtitles/'))
        
        # First check if the file has subtitle streams
        stdout, stderr, probe_status = sp(['ffprobe', "-v", "error", "-select_streams", "s", 
                                          "-show_streams", self.path], check=False)
        
        if probe_status == 0 and stdout.strip():
            # The file has subtitle streams, try to extract them
            stdout, stderr, returnstatus = sp(['ffmpeg', '-y', "-i", self.path, '-f', 'srt', 
                                              "-map", "0:s:0", "-vsync", "0", self.raw_subtitles_path], check=False)
        else:
            # No subtitle streams found or error in probing, create empty subtitle files
            logger.info(f"No subtitle streams found in {self.basename}, creating empty subtitle files")
            with open(self.raw_subtitles_path, 'w') as f:
            ...
            with open(self.subtitles_path, 'w') as f:
                ...
                try:
                    ...
                    for line in lines.split("\n"):
                        ...
                        elif "AM" in line or "PM" in line or "No subtitles available" in line:
                            subtitle = line.replace(",", "").replace("\n", "")
                        else:
                            try:
                                sub_number = int(line)
                            except ValueError:  # blank line
                                if sub_number and start_time and end_time and subtitle:
                                    f.write("{},{},{},{}\n".format(sub_number, start_time, end_time, subtitle))
                                    sub_number = start_time = end_time = subtitle = ""
                except Exception as e:
                    logger.warning(f"Error processing subtitles for {self.basename}: {str(e)}")
                    f.write("1,00:00:00.000,00:00:01.000,No subtitles available\n")
            
            self._subtitles_extracted = True
        else:
            # If extraction failed, create an empty subtitles file
            logger.warning(f"Failed to extract subtitles from {self.basename}, creating empty subtitle files")
            with open(self.raw_subtitles_path, 'w') as f:
                f.write("1\n00:00:00,000 --> 00:00:01,000\nNo subtitles available\n\n")
            with open(self.subtitles_path, 'w') as f:
                f.write("subtitle_number,start_time,end_time,subtitle\n")
    ...
    def check_for_errors(self, etypes=('Tuncating packet', 'non monotonically increasing dts', 'Read error', 'no frame',
        ...
        if self._errors_checked and (not forcenew):
            return 0
        if not os.path.exists(conf.get('dirs', {}).get('errors', 'outputs/errors/')):
            os.makedirs(conf.get('dirs', {}).get('errors', 'outputs/errors/'))
    ...
    def upload_vid_metadata_to_db(self, file_loc="fs"):
        ...
        try:
            stdout, stderr, returncode = sp([os.path.join(conf.get('dirs', {}).get('scripts', 'scratch/scripts/'), "s3etag.sh"), self.path, "7"])
            hash = stdout[:32]
        except:
            hash = -1
        ...
        new_frame_stats_file = f'{conf.get("dirs", {}).get("frame_stats", "outputs/frame_stats/")}{self.name}_uuid.csv'
        ...
        new_packet_stats_file = f'{conf.get("dirs", {}).get("packet_stats", "outputs/packet_stats/")}{self.name}_uuid.csv'
        ...
        new_subtitles_file = f'{conf.get("dirs", {}).get("subtitles", "outputs/subtitles/")}{self.name}_uuid.csv'
...
if __name__ == "__main__":
    setup("video_file")
    vid_file = glob(os.path.join(conf.get('dirs', {}).get('raw_videos', 'downloaded_videos'), "*.mkv"))[0]
